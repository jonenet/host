...
public class RePlugin {
    ...
    public static class App {
        ...
        public static void attachBaseContext(Application app, RePluginConfig config) {
            ...
            //一、、初始化进程间通信辅助类,因为不同进程的创建会使attachBaseContext方法走多次,
            //IPC.init中会标记当前进程类型，将会影响下面的代码在不同进程中的逻辑

            // 4.IPC的代码初始化就这么多，
            // 主要就是通过proc文件获取当前进程名、进程id和宿主包名,然后设置常驻进程的名称，
            // 最后标记当前进程是否是ui进程和是不是常驻进程，我们知道每一个进程在创建后都会调用该进程的ActivityThread的main方法，
            // 开启消息循环，绑定AMS，创建Application，加载provider，然后回调Application生命周期方法。所以说这里会被调用多次
            // ，多次调用导致了IPC中判断当前是否是插件管理进程时的值不同而影响后面的执行逻辑不同。
            IPC.init(app);
            ...
            //就是在编译期间replugin-host-gradle自动生成的 RePluginHostConfig,这个方法是一个空方法，反射初始化的逻辑在static语句块中
